...
if (command == 'reg' || command == 'verificar' || command == 'Registrar') {
    ...
    if (user.registered === true) return m.reply('*Tu Ya Estas Registrado, o No Lo Recuerdas?*') 
    if (!Reg.test(text)) return conn.sendMessage(m.chat, {video: {url: verificar}, caption: `*❌ 𝙵𝙾𝚁𝙼𝙰𝚃𝙾𝚂 𝙸𝙽𝙲𝙾𝚁𝚁𝙴𝙲𝚃𝙾*\n\n*PORFAVOR ESCRIBE BIEN EL FORMATO DE REGISTRO:*\n${prefix}reg *Nombre|Edad* \n_Con el *|* que los divide._`}, {quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
    ...
    conn.sendMessage(m.chat, {
        video: {url: verificados2}, 
        caption: `┌────「 *ⱤɆ₲ł₴₮Ɽ₳ĐØ* 」─
(⛦) *ᴜsᴇʀ*: @${sender.split("@")[0]}
(⛦) *ɴᴏᴍʙʀᴇ:* _${name}_
(⛦) *ɴᴜᴍᴇʀᴏ:* _wa.me/${sender.split("@")[0]}_
(⛦) *ᴇᴅᴀᴅ:* _${age} años_
(⛦) *ʜᴏʀᴀ:* _${time}_
(⛦) *𝙿𝙰𝙸𝚂:* _${userNationality}_
(⛦) *ғᴇᴄʜᴀ:* _${date}_
(⛦) *sᴇʀɪᴀʟ:* _${sn}_
(⛦) *ʀᴇᴄᴏᴍᴘᴇɴsᴀ:* _5 crédito + 600 exp_
└────「🔥 ${botname} 🔥」\n
ᴠᴇʀɪғɪᴄᴀᴄɪᴏɴ ᴄᴏᴍᴘʟᴇᴛᴀ ᴜsᴀ *${prefix}ᴍᴇɴᴜ* ᴘᴀʀᴀ ᴠᴇʀ ᴇʟ ᴍᴇɴᴜ💯`, 
        mentionedJid: [name]
    ...
    await delay(2 * 2000)
    conn.sendMessage(m.chat, { text: '*💬 Elegir tu idioma con:* #idioma', contextInfo: {forwardingScore: 9999999, isForwarded: false}}, { quoted: m, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
}
...
let count = command.replace(/^lobo/i, '');
count = count ? /all/i.test(count) ? Math.floor(global.db.data.users[m.sender].limit / 100) : parseInt(count) : args[0] ? parseInt(args[0]) : 1;
count = Math.max(1, count);
if (global.db.data.users[m.sender].lobo > 1) return m.reply(`Ya tiene 1 lobos`) 
if (global.db.data.users[m.sender].limit >= 100 * count) {
global.db.data.users[m.sender].limit -= 100 * count;
global.db.data.users[m.sender].lobo += count;
m.reply(`🥳𝐅𝐞𝐥𝐢𝐜𝐢𝐝𝐚𝐝𝐞𝐬! 𝐇𝐚𝐬 𝐚𝐝𝐪𝐮𝐢𝐫𝐢𝐝𝐨🥳
 ᴀ ${count} ʟᴏʙᴏ🐺 
...
let count = command.replace(/^mono/i, '');
count = count ? /all/i.test(count) ? Math.floor(global.db.data.users[m.sender].limit / 100) : parseInt(count) : args[0] ? parseInt(args[0]) : 1;
count = Math.max(1, count);
if (global.db.data.users[m.sender].mono > 1) return m.reply(`Ya tiene 1 monos🐵`) 
if (global.db.data.users[m.sender].limit >= 100 * count) {
global.db.data.users[m.sender].limit -= 100 * count;
global.db.data.users[m.sender].mono += count;
m.reply(`🥳𝐅𝐞𝐥𝐢𝐜𝐢𝐝𝐚𝐝𝐞𝐬! 𝐇𝐚𝐬 𝐚𝐝𝐪𝐮𝐢𝐫𝐢𝐝𝐨🥳
...
count = count ? /all/i.test(count) ? Math.floor(global.db.data.users[m.sender].limit / 100) : parseInt(count) : args[0] ? parseInt(args[0]) : 1;
count = Math.max(1, count);
if (global.db.data.users[m.sender].gato > 1) return m.reply(`Ya tiene 1 gatos `) 
if (global.db.data.users[m.sender].limit >= 100 * count) {
global.db.data.users[m.sender].limit -= 100 * count;
