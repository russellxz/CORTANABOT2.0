(async () => {
  ...
  async function getPrompt() {
    try {
      const res = await fetch(
        "https://raw.githubusercontent.com/elrebelde21/LoliBot-MD/main/src/text-chatgpt.txt"
      );
  ...
  // ✅ Baileys es ESM: cargar con import() en CJS
  const {
  ...
  let method = "1"; // QR por defecto
  if (!fs.existsSync("./sessions/creds.json")) {
    method = await question(chalk.magenta("👉 Elige método (1 = QR, 2 = Código 8 dígitos): "));
  ...
  async function startBot() {
    try {
      ...
      const browser =
        method === "1"
          ? (Browsers?.macOS?.("Safari") ?? ["macOS", "Safari", "2.3000.1029030078"])
      ...
      // Vinculación por código (solo si método = 2 y no hay sesión)
      if (!fs.existsSync("./sessions/creds.json") && method === "2") {
        let phoneNumber = await question(
          chalk.magenta("📞 Ingresa tu número con código de país (ej: 5491168XXXX): ")
        );
        phoneNumber = phoneNumber.replace(/\D/g, "");

        // ✅ Delay de 2s como pediste
        setTimeout(async () => {
