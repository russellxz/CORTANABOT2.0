// index.js — Parche ESM-safe + rutas tiempo actualizadas, sin cambiar tu flujo
...
let canalNombre = ["🪼 Cortana Ultra 2.0 BOT 🪼"];
...
// ⬇️ INYECCIÓN GLOBAL PARA PLUGINS
  ...
  async function startBot() {
    try {
      ...
      // pairing 8 dígitos (sin tocar tu flujo)
      ...
      // ---------- TUS BLOQUES MANTENIDOS (con ajustes mínimos) ----------
      ...
      // === REVISIÓN AUTOMÁTICA DE GRUPOS (rutas nuevas) ===
      setInterval(async () => {
        try {
            ...
            for (const groupId of Object.keys(tiempoCerrar)) {
              ...
              if (ahora >= t) {
                ...
                try {
                  ...
                  await sock.sendMessage(groupId, { text: "🔒 El grupo ha sido cerrado automáticamente. Solo admins pueden escribir." });
                } catch (e) {
                  console.error(`❌ Error cerrando grupo ${groupId}:`, e);
                }
            ...
            for (const groupId of Object.keys(tiempoAbrir)) {
              ...
              if (ahora >= t) {
                ...
                try {
                  ...
                  await sock.sendMessage(groupId, { text: "🔓 El grupo ha sido abierto automáticamente. ¡Todos pueden escribir!" });
                } catch (e) {
                  console.error(`❌ Error abriendo grupo ${groupId}:`, e);
                }
      ...
      // ====== BIENVENIDA / DESPEDIDA + ANTIARABE (igual a tu lógica) ======
        ...
        try {
          ...
          if (update.action === "add" && welcomeActivo) {
            for (const participant of update.participants) {
              ...
              if (customMessage) {
                await sock.sendMessage(update.id, { image: { url: profilePicUrl }, caption: `👋 ${mention}\n\n${customMessage}`, mentions: [participant] });
              } else {
                ...
                if (option < 0.33) {
                  ...
                  await sock.sendMessage(update.id, { text: `👋 ${mention}\n\n${mensajeTexto}${groupDesc}`, mentions: [participant] });
                } else {
                  await sock.sendMessage(update.id, { text: `👋 ${mention}\n\n${mensajeTexto}`, mentions: [participant] });
                }
              }
      ...
      // ====== MENSAJES / TODA TU LÓGICA (sin tocar) ======
      sock.ev.on("messages.upsert", async (messageUpsert) => {
        try {
          ...
          } catch (e) { console.error("❌ Error en lógica antiprivado:", e); }
          ...
          if (isGroup && activos.antis?.[chatId] && !fromMe && stickerMsg) {
            ...
            if (userData.count > 5 && timePassed < 15000) {
              ...
              if (userData.strikes >= 3) {
                await sock.sendMessage(chatId, { text: `❌ @${user.split("@")[0]} fue eliminado por ignorar las advertencias y abusar de los stickers.`, mentions: [user] });
          ...
          } catch (e) { console.error("❌ Error al guardar mensaje antidelete:", e); }
          ...
          if (msg.message?.protocolMessage?.type === 0) {
            try {
              ...
              if (deletedData.media) {
                ...
                if (type === "sticker") {
                  ...
                  await sock.sendMessage(chatId, { text: `📌 El sticker fue eliminado por @${senderNumber}`, mentions: mentionTag, quoted: sent });
                } else if (type === "audio") {
                  const sent = await sock.sendMessage(chatId, sendOpts);
                  await sock.sendMessage(chatId, { text: `🎧 El audio fue eliminado por @${senderNumber}`, mentions: mentionTag, quoted: sent });
                } else {
            ...
            } catch (err) { console.error("❌ Error en lógica antidelete:", err); }
          }
          ...
          } catch (e) { console.error("❌ Error en lógica ChatGPT por grupo:", e); }
          ...
          try {
            ...
            if (isGroup && lumiActivo && !fromMe && textMsg.length > 0) {
              ...
              try { result = await luminaiQuery(textMsg, name, prompt); result = cleanResponse(result); }
                ...
                catch (e2) { console.error("Error Perplexity:", e2); result = "❌ No se obtuvo respuesta de los servicios"; }
              }
              if (result) await sock.sendMessage(chatId, { text: result }, { quoted: msg });
            }
          } catch (e) { console.error("❌ Error en lógica Lumi AI automática:", e); }
          ...
          } catch (e) { console.error("❌ Error en contador de mensajes:", e); }
          ...
          } catch (err) { console.error("❌ Error al ejecutar comando desde sticker:", err); }
          ...
          } catch (e) { console.error("❌ Error en lógica de palabra clave:", e); }
          ...
          try {
            ...
            if (isGroup) {
              ...
              if (muteList.includes(senderId)) {
                ...
                if (count === 8) await sock.sendMessage(chatId, { text: `⚠️ @${senderId.split("@")[0]} estás muteado.\nSigue enviando mensajes y podrías ser eliminado.`, mentions: [senderId] });
                if (count === 13) await sock.sendMessage(chatId, { text: `⛔ @${senderId.split("@")[0]} estás al límite.\nSi envías *otro mensaje*, serás eliminado del grupo.`, mentions: [senderId] });
                if (count >= 15) {
                  const metadata = await sock.groupMetadata(chatId);
                  ...
                  if (!isAdmin) {
                    ...
                    await sock.sendMessage(chatId, { text: `❌ @${senderId.split("@")[0]} fue eliminado por ignorar el mute.`, mentions: [senderId] });
                    delete global._muteCounter[key];
                  } else {
                    await sock.sendMessage(chatId, { text: `🔇 @${senderId.split("@")[0]} es administrador y no se puede eliminar.`, mentions: [senderId] });
                  }
                }
          ...
          } catch (err) { console.error("❌ Error en lógica de muteo:", err); }
          ...
          } catch (e) { console.error("❌ Error procesando bloqueo de usuarios baneados:", e); }
          ...
          try {
            ...
            if (!isGroup) {
              ...
              if (esArabe) {
                ...
                await sock.sendMessage(myJid, { text: `📛 *Número árabe bloqueado automáticamente:*\n\n🧿 Número: wa.me/${senderNum}\n📩 Intentó escribir al bot en privado.\n\n✅ El número fue bloqueado.` });
                await sock.sendMessage(sender, { text: "🚫 Este bot no acepta mensajes privados de números árabes. Has sido bloqueado automáticamente." });
                return;
              }
            }
          } catch (e) { console.error("❌ Error en bloqueo automático de árabes:", e); }

          // 🔐 Modo Privado y Admins por grupo (tu lógica intacta)
          ...
          } catch (e) { console.error("❌ Error en lógica de bloqueo por apagado:", e); }

          // Bloqueo comandos RPG si inactivo
          try {
            const comandosRpg = ["rpg","nivel","picar","minar","minar2","work","crime","robar","cofre","claim","batallauser","hospital","hosp","luchar","poder","volar","otromundo","otrouniverso","mododios","mododiablo","podermaximo","enemigos","nivelper","per","bolasdeldragon","vender","quitarventa","batallaanime","comprar","tiendaper","alaventa","verper","daragua","darcariño","darcomida","presumir","cazar","entrenar","pasear","supermascota","mascota","curar","nivelmascota","batallamascota","compra","tiendamascotas","vermascotas","addmascota","addper","deleteuser","deleteper","deletemascota","totalper","tran","transferir","dame","dep","bal","saldo","retirar","depositar","delrpg","topuser","topmascotas","topper"];
          ...
          } catch (e) { console.error("❌ Error procesando bloqueo de comandos RPG:", e); }
          ...
          } catch (e) { console.error("❌ Error procesando bloqueo de modo caliente:", e); }
          ...
          try {
            ...
            if (comandosRestringidos.includes(commandOnly) && !isOwnerX && !isFromMe) {
              ...
              if (cacheData[key] < 5) {
                await sock.sendMessage(chatId, { text: `🚫 *Este comando está restringido en este grupo.*\n⚠️ Solo el owner o el bot pueden usarlo.` }, replyOptions);
              } else if (cacheData[key] === 5) {
                await sock.sendMessage(chatId, { text: `❌ *Has intentado usar este comando demasiadas veces.*\n🤖 Ahora el bot te ignorará respecto a *${commandOnly}*.` }, replyOptions);
              }
              fs.writeFileSync(cachePath, JSON.stringify(cacheData, null, 2));
              return;
            }
          } catch (e) { console.error("❌ Error procesando comando restringido:", e); }
      ...
      sock.ev.on("connection.update", async (update) => {
        ...
        if (connection === "connecting") {
          ...
          if (fs.existsSync(restarterFile)) {
            ...
            } catch (error) { console.error("❌ Error al procesar lastRestarter.json:", error); }
          }
      ...
      process.on("unhandledRejection", (reason, promise) => console.error(chalk.red("🚨 Promesa rechazada sin manejar:"), promise, "razón:", reason));
      // ---------- FIN BLOQUES ----------
    } catch (error) {
